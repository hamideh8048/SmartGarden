...
import 'package:samrt_garden/widget/FertilizeWidget.dart';
import 'package:samrt_garden/widget/LampWidget.dart';
import 'package:samrt_garden/widget/PumpWidget.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:udp/udp.dart';

import '../widget/WaterWidget.dart';

class GardenDetailsScreen extends StatefulWidget {
  const GardenDetailsScreen({Key? key,required this.flowerModel}) : super(key: key);

  final  FlowerModel flowerModel;
  @override
  State<StatefulWidget> createState() {
    return GardenDetailsScreenState();
  }
}

class GardenDetailsScreenState extends State<GardenDetailsScreen>
 ...
 userplantList= plantList.where((elem) => plantliststr.contains(elem.id)).toList();
  ...
  try {
    ...
    if (token != null && token.length > 2) {
      ...
      var response;
      if (isFertilize) {
        response = await (Services()).registerUpdateFlowerpotfertilize(
            token,
            fmodel!.serial_number,
            fmodel!.fertilize_status,
            fmodel!.light_status,
            fmodel!.pump_status,
            fmodel!.water_status,
            pumpValue,
        ...
        response = await (Services()).registerUpdateFlowerpot(
            token,
            fmodel!.serial_number,
            fmodel!.fertilize_status,
            fmodel!.light_status,
      ...
      if (response != null && response["id"] != null) {
        ...
        if (fmodel!.plants.length > 0)
          userplantList =
              plantList.where((elem) => plantliststr.contains(elem.id))
                  .toList();
...
CheckDropdown()
{
  if(fmodel!.pump_interval_minutes.contains("15")) {
    pumpDropdownValue = 'هر 15 دقیقه';
    pumpValue="15";
  }
  else
  if(fmodel!.pump_interval_minutes.contains("20"))
    pumpDropdownValue='هر 20 دقیقه';
  else
  if(fmodel!.pump_interval_minutes.contains("30"))
    pumpDropdownValue='هر 30 دقیقه';

  if(fmodel!.light_hours.contains("9"))
    lightDropdownValue='هر 9 ساعت';
  else
  if(fmodel!.light_hours.contains("12"))
    lightDropdownValue='هر 12 ساعت';
  else
  if(fmodel!.light_hours.contains("15"))
    lightDropdownValue='هر 15 ساعت';
  else
  if(fmodel!.light_hours.contains("18"))
    lightDropdownValue='هر 18 ساعت';
}
String pumpValue="";
String lightValue="";
  getDropdown()
  ...
  Future<void> getFlowers() async {
    ...
    // if(preferences.getString('FLOWER2') != null){
    //   flower2 = preferences.getString('FLOWER2')!;
    //   setState(() {
    //     flower2;
    //   });
    // }
    // if(preferences.getString('FLOWER3') != null){
    //   flower3 = preferences.getString('FLOWER3')!;
    //   setState(() {
    //     flower3;
    //   });
    // }
...
SizedBox(
  height: 10,
                              ...
                              Container(
                                height:userplantList.length<5? 90:190,child:
                          ...
                          //         (i) =>
                              GridView.count(
                                  ...
                                  crossAxisSpacing: 1,
                                  mainAxisSpacing: 10,
                                  children: List.generate(userplantList.length, (index) {
                                    return  Padding(
                                ...
                                CachedNetworkImage(
                                  ...
                                  imageUrl: userplantList[index].url,
                    ...
                    //  getFirstWidget(isChange),
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            Padding(
                              padding: EdgeInsets.only(left: 20,right: 12,top: 12,bottom: 12),
                              child: WaterWidget(
                        itsOn: fmodel!.water_status,
                        switchCenter: false,
                        switchButton: lightSwitch,
                        onTap: () {
                        //  fmodel!.water_status=! fmodel!.water_status;
                          sendLight();
                        ...
                        iconAsset: 'assets/icons/watericon.png',
                        device: 'آب',
                      ),

                              // Switch(
                              //   value: fmodel!.water_status,
                              //   onChanged: (bool value) {
                              //     print('before send udp');
                              //       setState(() {
                              //         isEnought=!isEnought;
                    ...
                    child: Row(
                      ...
                      children: [
                        Padding(
                          ...
                          child:
                          FertilizeWidget(
                            itsOn: fmodel!.fertilize_status,
                            switchCenter: false,
                            switchButton: lightSwitch,
                            onTap: () {
                              fmodel!.fertilize_status=! fmodel!.fertilize_status;
                              sendLight();
                            ...
                            iconAsset: 'assets/icons/fertilizericon.png',
                            device: 'آب',
                          ),
                          // Switch(
                          //   value: fmodel!.fertilize_status,
                          //   onChanged: (bool value) {
                          //
                          //     setState(() {
                        ...
                        Padding(
                          ...
                          Text(
                            ...
                            Text(
                            'هر زمان کود اضافه کردید دکمه را فشار دهید',
